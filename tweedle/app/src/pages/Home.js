...
function Home(){
    let [likeText, setLikeText] = useState("Tweedle likes this");
    let {like, getLikeCount} = useAPI();
    let {likes, likesCount} = useSelector(state => state.user);


    useEffect(()=>{
        if(likesCount > 1){
            let s = (likesCount-1)>1 ? "s" : ""
            if(likes){
                setLikeText(`You and ${likesCount-1} other${s} like this`)
            }else{
                setLikeText(`Tweedle and ${likesCount-1} other${s} like this`)
            }
        }else{
            setLikeText("Tweedle likes this")
        }
    },[likes,likesCount])
    ...
    useEffect(()=>{
        getLikeCount();
        let id = setInterval(getLikeCount,5000);
    ...
    return(
        <div className='h-screen flex items-center px-6'>     
            <div className="pb-20 lg:w-1/2">
                <h1 className="text-3xl font-varela mb-6 md:text-5xl">The Game Of Tweets</h1>
                <p className="mb-2 font-medium text-sm md:text-base">
                   {likeText}
                </p>
                <div className="flex flex-row gap-4">
                    <img src={logo} className='w-12 self-start' alt="tweedle dp"/>
                    <div>
                        <p className="font-medium text-sm md:text-base">Tweedle<span className="inline-block ml-1 font-light">@tweedle</span></p>
                        <p className="mb-6 md:text-lg">Welcome to Tweedle. Answer questions on popular realtime tweets, score the most points and win trophies.</p>
                        <div className="flex flex-row justify-between items-center text-sm mb-8">
                            <FaRegCommentAlt className="text-xl md:text-2xl opacity-50"/>
                            <AiOutlineRetweet className="text-2xl md:text-3xl opacity-50"/>
                            <div onClick={like} className='cursor-pointer'>
                            {likes ? <FaHeart className="text-xl md:text-2xl text-pink-500"/>
                            :<FaRegHeart className="text-xl md:text-2xl"/>}
                            </div>
                        </div>
                    </div>
                </div>
                <Link to={'quiz'} className="rounded-full ring-0 ring-white py-4 px-10 
                hover:bg-white hover:text-light-blue font-medium text-center
                w-full block md:w-1/2 bg-accent">
                    Play Now
                </Link>
            </div>
